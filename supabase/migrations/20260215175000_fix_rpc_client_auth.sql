-- Fix RPC client authorization check
-- Problem: we were checking ss.client_id = auth.uid(), but ss.client_id refers to clients.id, not auth.users.id.
-- Solution: Join with clients table to check auth_id.
...
AS $$
BEGIN
    -- Verify access by joining with clients table
    ...
    IF NOT EXISTS (
        ...
        -- Allow if user is a coach (optional fallback)
        IF NOT EXISTS (

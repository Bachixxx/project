...
export function SortableExercise({
    ...
    return (
        <div ref={setNodeRef} style={style} className={`relative mb-2 ${isLinkedToNext ? 'pb-0' : ''}`}>
            {/* Connection Line */}
            {isLinkedToNext && (
                <div className="absolute left-6 top-full h-4 w-0.5 bg-blue-500/50 -translate-x-1/2 z-0" />
            )}

            <div className={`p-3 bg-white/5 rounded-lg border ${isLinkedToPrev || isLinkedToNext ? 'border-blue-500/30' : 'border-white/5'} group relative`}>
                ...
                <div className="grid grid-cols-12 gap-2 items-center">
                    ...
                    {isLinkedToNext && (
                        <div className="col-span-3 sm:col-span-3 flex items-center justify-center text-xs text-blue-400 font-medium italic">
                            Enchaîné
                        </div>
                    )}
                </div>
                ...
                {onLink && !isLinkedToNext && (
                    <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                        <button
                            ...
                            title="Lier avec l'exercice suivant"
                        >
                ...
                {/* Unlink Action Button (if linked to next) */}
                {isLinkedToNext && onUnlink && (
                    <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                        <button
                            onClick={onUnlink}
